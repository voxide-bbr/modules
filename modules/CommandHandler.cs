...
public class CommandHandler : BattleBitModule
{
    // Edit 2 of 3
    [ModuleReference]
    public dynamic? DiscordWebhooks { get; set; } = null!;
    ...
    private void handleCommand(RunnerPlayer? player, string message)
    {
        ...
        {
            if (player != null)
                this.DiscordWebhooks.SendMessage($":rocket: [{player.SteamID}] {player.Name} ran command \"!{string.Join(" ",fullCommand)}\"");
            else
                this.DiscordWebhooks.SendMessage($":rocket: Ran command \"!{string.Join(" ", fullCommand)}\"");
        }
