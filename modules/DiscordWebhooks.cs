...
// I temporarily neutered this file because discord api rate limits were being hit
// Edit 1 of 4
using System.Net;
...
public class DiscordWebhooks : BattleBitModule
{
    ...
    {
        this.discordMessageQueue.Enqueue(new VoxideJoinAndLeaveMessage(Server.CurrentPlayerCount, player.Name, player.SteamID, player.IP, true));
        return Task.CompletedTask;
    }
    public override Task OnPlayerDisconnected(RunnerPlayer player)
    {
        this.discordMessageQueue.Enqueue(new VoxideJoinAndLeaveMessage(Server.CurrentPlayerCount, player.Name, player.SteamID, player.IP, false));
        return Task.CompletedTask;
    }
...
internal class VoxideJoinAndLeaveMessage : DiscordMessage
{
    public int PlayerCount { get; set; }

    public VoxideJoinAndLeaveMessage(int playerCount, string playerName, ulong steamID, IPAddress ipAddress, bool joined)
    {
    ...
    {
        return $"{(this.Joined ? ":arrow_right:" : ":arrow_left:")} [{this.SteamID}] [||{this.IPAddress}||] {this.PlayerName} {(this.Joined ? "joined" : "left")} ({this.PlayerCount} players)";
    }
}
